...
final class OSGeolocation: CDVPlugin {
    ...
    func getLocation(command: CDVInvokedUrlCommand) {
        guard let config: OSGeolocationCurrentPositionModel = self.createModel(for: command.argument(at: 0))
        else {
    ...
    func addWatch(command: CDVInvokedUrlCommand) {
        guard let config: OSGeolocationWatchPositionModel = self.createModel(for: command.argument(at: 0))
        else {
            callbackManager?.sendError(.inputArgumentsIssue(target: .watchPosition))
            return
        }
        handleLocationRequest(config.enableHighAccuracy, timerId: config.id, command.callbackId)
    }

    @objc
    func clearWatch(command: CDVInvokedUrlCommand) {
        guard let config: OSGeolocationClearWatchModel = self.createModel(for: command.argument(at: 0))
        else {
...
private extension OSGeolocation {
    ...
    func handleLocationRequest(_ enableHighAccuracy: Bool, timerId: String? = nil, _ callbackId: String) {
        ...
        if let timerId {
            callbackManager?.addWatchCallback(String(timerId), callbackId)
        } else {
